...
/**
 * Auto-generated InventoryItem Routes
 *
...
import type { InventoryItemHooks } from "./inventory-item.hooks"
...
/**
 * Set hooks for InventoryItem routes
 */
export function setInventoryItemHooks(h: InventoryItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "quantity", "unit", "minStock", "cost", "supplier", "lastUpdated", "createdAt"] as const
const RELATION_FIELDS = ["transactions"] as const
...
/**
 * Create InventoryItem CRUD routes
 */
export function createInventoryItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[InventoryItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "InventoryItem not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[InventoryItem] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[InventoryItem] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[InventoryItem] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[InventoryItem] Delete error:", error)
