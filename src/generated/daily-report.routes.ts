...
/**
 * Auto-generated DailyReport Routes
 *
...
import type { DailyReportHooks } from "./daily-report.hooks"
...
/**
 * Set hooks for DailyReport routes
 */
export function setDailyReportHooks(h: DailyReportHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["date", "totalOrders", "totalRevenue", "cashTotal", "cardTotal", "onlineTotal", "deliveryOrders", "dineInOrders", "takeawayOrders", "generatedAt"] as const
...
/**
 * Create DailyReport CRUD routes
 */
export function createDailyReportRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[DailyReport] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "DailyReport not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[DailyReport] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[DailyReport] Update error:", error)
