...
import type { TableHooks } from "./table.hooks"
...
/**
 * Set hooks for Table routes
 */
export function setTableHooks(h: TableHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["number", "capacity", "status", "section", "createdAt"] as const
const RELATION_FIELDS = ["orders"] as const
...
/**
 * Create Table CRUD routes
 */
export function createTableRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Table not found" } }, 404)
      }
