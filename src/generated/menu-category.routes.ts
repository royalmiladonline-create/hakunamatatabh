...
/**
 * Auto-generated MenuCategory Routes
 *
...
import type { MenuCategoryHooks } from "./menu-category.hooks"
...
/**
 * Set hooks for MenuCategory routes
 */
export function setMenuCategoryHooks(h: MenuCategoryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "nameAr", "displayOrder", "isActive", "createdAt"] as const
const RELATION_FIELDS = ["items"] as const
...
/**
 * Create MenuCategory CRUD routes
 */
export function createMenuCategoryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[MenuCategory] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.menuCategory.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "MenuCategory not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[MenuCategory] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[MenuCategory] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[MenuCategory] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[MenuCategory] Delete error:", error)
