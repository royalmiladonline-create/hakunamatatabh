...
/**
 * Auto-generated MenuItem Routes
 *
...
import type { MenuItemHooks } from "./menu-item.hooks"
...
/**
 * Set hooks for MenuItem routes
 */
export function setMenuItemHooks(h: MenuItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "nameAr", "description", "price", "currency", "isAvailable", "image", "createdAt", "updatedAt", "categoryId"] as const
const RELATION_FIELDS = ["category", "orderItems"] as const
...
/**
 * Create MenuItem CRUD routes
 */
export function createMenuItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[MenuItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "MenuItem not found" } }, 404)
      }
