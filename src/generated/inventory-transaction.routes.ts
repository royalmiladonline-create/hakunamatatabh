...
/**
 * Auto-generated InventoryTransaction Routes
 *
...
import type { InventoryTransactionHooks } from "./inventory-transaction.hooks"
...
let hooks: InventoryTransactionHooks = {}
...
/**
 * Set hooks for InventoryTransaction routes
 */
export function setInventoryTransactionHooks(h: InventoryTransactionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["type", "quantity", "notes", "createdAt", "itemId"] as const
...
/**
 * Create InventoryTransaction CRUD routes
 */
export function createInventoryTransactionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[InventoryTransaction] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.inventoryTransaction.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "InventoryTransaction not found" } }, 404)
      }
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[InventoryTransaction] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.inventoryTransaction.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[InventoryTransaction] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[InventoryTransaction] Delete error:", error)
